...
const PoliceDashboard = () => {
  ...
  const fetchIncidents = async () => {
    try {
      setLoading(true);
      const res = await fetch(`${API_ROUTES.base}/api/incidents?recipient=police`, {
        headers: {
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.header}>Incident Reports for Police</Text>
      {loading ? (
        <ActivityIndicator size="large" color="#204080" />
      ) : (
        <FlatList
          data={incidents}
...
const styles = StyleSheet.create({
  ...
  header: { fontSize: 20, fontWeight: 'bold', marginBottom: 16, color: '#204080' },
  ...
  incidentType: { fontSize: 16, fontWeight: 'bold', color: '#204080' },
