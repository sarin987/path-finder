...
const AmbulanceDashboard = () => {
  ...
  const fetchIncidents = async () => {
    try {
      setLoading(true);
      const res = await fetch(`${API_ROUTES.base}/api/incidents?recipient=ambulance`, {
        headers: {
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.header}>Incident Reports for Ambulance</Text>
      {loading ? (
...
const styles = StyleSheet.create({
  ...
  header: { fontSize: 20, fontWeight: 'bold', marginBottom: 16, color: '#d9534f' },
  ...
  incidentType: { fontSize: 16, fontWeight: 'bold', color: '#d9534f' },
